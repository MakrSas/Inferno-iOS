#!/bin/bash
# Builds and signs arun — the guest-side audio probe. The entitlements are
...
xcrun --sdk iphoneos clang -target arm64-apple-ios14.0 -isysroot "$SDK" -Os \
    -framework Foundation -framework IOKit -framework AudioToolbox -o "$OUT" "$HERE/arun.m"
strip -x "$OUT"
ldid "-S$HERE/arun.entitlements.plist" "$OUT"
echo "built: $OUT"
